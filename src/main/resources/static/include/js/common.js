 ...
 * 
 * @param arr :
 *            비어있는지 체크할 폼 요소 id, 얼럿 메시지 쌍의 배열 ex) checkNotEmpty[
 *            ["elem1","메시지1"], ["elem2","메시지2"] ]
 * 
 ...
 * 
 * @param arr :
 *            비어있는지 체크할 폼 요소 id, 얼럿 메시지 쌍의 배열 ex) checkNotValue[
 *            ["elem1","메시지1"], ["elem2","메시지2"] ]
 * 
 * @return : 값이 비어있는 폼 요소가 하나라도 있으면 false 값이 전부 입력된 경우 true
 */
 ...
 *            페이지 번호 갯수
 * @param pageFunc
 *            페이지 번호를 클릭하면 호출할 함수 객체
 * @param exParams
 *            pageFunc에 넘겨줄 추가적인 파라미터 ( optional / 가능한 파라미터 형식: 문자열 )
 * @return html 문자열
 */
function getPaginationHtml(currentPage, totalCount, pageRow, blockPage,
		pageFunc, exParams) {
	...
	pagingHtml += "<a class='first' href='javascript:" + pageFunc + "(1"
			+ sExParam + ")'><span class='hidden'>맨앞</span></a>";
	pagingHtml += "<a class='pre' href='javascript:" + pageFunc + "("
			+ (startPage - 1 == 0 ? 1 : (startPage - 1)) + sExParam
	...
	for (var i = startPage; i <= endPage; i++) {
		...
		if (i == currentPage) {
			...
			pagingHtml += " <a href=javascript:" + pageFunc + "(" + i
					+ sExParam + ")>" + i + "</a>";
		}
	}

	pagingHtml += "<a class='next' href='javascript:" + pageFunc + "("
			+ (endPage + 1 > totalPage ? totalPage : (endPage + 1)) + sExParam
			+ ")'><span class='hidden'>다음</span></a>";
	pagingHtml += "<a class='last' href='javascript:" + pageFunc + "("
			+ totalPage + sExParam + ")'><span class='hidden'>맨뒤</span></a>";
	pagingHtml += "</div>";
...
/**
 * 텍스트 박스에 숫자만 입력 onKeyDown으로만 사용 가능 예) <input type="text" style="ime-mode:
 * disabled;" onkeydown="fFilterNumber(event);">
 * 
 * @param event
 *            Key 이벤트
 * @return true, false
...
/**
 * 텍스트 박스에 영문, 숫자만 입력 onKeyDown으로만 사용 가능 예) <input type="text"
 * style="ime-mode:disabled;" onkeydown="fFilterAlphaNumber(event);">
 * 
 * @param event
 *            Key 이벤트
 * @return true, false
...
/**
 * 텍스트 박스에 입력된 값이 날짜 형식인지 체크 한다 onKeyDown 또는 onKeyUp으로 사용 가능 예) <input
 * type="text" onKeyUp="javascript:g_fFilterDate(this);"> 확장 예)<input
 * type="text" maxlength="10" onKeyDown="javascript:fFilterNumber(event);"
 * onKeyUp="javascript:g_fFilterDate(this);" />
 * 
 * @param this
 *            object
 * @return -
...
function g_fFilterDate(obj) {
	...
	if (strLen == 8) {
		/*
		 * var format =
		 * /^(19[5-9][0-9]|20\d{2})(0[0-9]|1[0-2])(0[1-9]|[1-2][0-9]|3[0-1])$/;
		 */
...
function fShowPopupImg(url, param) {
	...
	var frmHtml = [];
	frmHtml.push('<form id="' + targetFrm + '" name="' + targetFrm
			+ '" action="' + url + '" >');
...
function fOnloadImg(param) {
	...
	callAjax("/image/selectImgSize.do", "post", "json", false, param,
			resultCallback);
}
/**
 * fOnloadImg 콜백함수
...
function fOnloadImgResult(data) {
	...
	var left = (screen.width - width) / 2;
	var top = (screen.height - height) / 2;
	var OpenWindow = window.open('', 'showPopup', 'left=' + left + ',top='
			+ top + ',width=' + width + ', height=' + height
			+ ', menubars=no, scrollbars=auto');
	OpenWindow.document.title = "fOnloadImgResult";
	OpenWindow.document
			.write("<style>body{margin:0px;}</style><img src='/image/showImg.do?fileName="
					+ data.url + "'>");
...
/**
 * fChkId 아이디는 영문, 숫자를 포함한 7자리이상 20자리 미만의 값 만 가질수 있다.
 * 
 * @param value
 * @returns boolean
 * 
	...
	new daum.Postcode({
		oncomplete : function(data) {
			...
			if (id == "zipFrame") {
				...
				document.getElementById(zcd).value = data.zonecode; // 5자리 새우편번호
				// 사용
				document.getElementById(adrs).value = fullAddr;
				document.getElementById(dtlAdrs).focus();
				// document.getElementById('sample2_addressEnglish').value =
				// data.addressEnglish;
			} else {
...
function g_fMoveWrqInfo(value) {
	...
	var targetFrm = 'goFrm';
	var frmHtml = [];
	frmHtml.push('<form id="' + targetFrm + '" action="' + action
			+ '" method="POST">');
...
/**
 * fChkByte 바이트수를 체크하여 넘으면 경고를 보낸다.
 * 
 * @param obj :
 *            내용, maxByte : 최대 바이트, id : 바이트diplay span
 * @returns boolean
 * 
 */
...
function fConverDateString(dt, gbn) {
	return dt.getFullYear() + gbn + fAddZero(eval(dt.getMonth() + 1)) + gbn
			+ fAddZero(dt.getDate());
}
/**
 * fAddZero
...
/**
 * 정수형 문자열을 숫자로 변환 - 숫자를 제외한 모든 문자 삭제 후 변환
 * 
 * @param str
 * @returns {Number}
 */
...
function g_fViewPcsDgsFil(wrq_id) {
	...
	jQuery(
			"<form action='/makMgr/downloadPcsDgsFil.do' method='post'>"
...
function g_fPopDentViewer(ofcId, ptetId, wrqId, focusId) {
	...
	if (ofcId == null || ofcId == "" || ofcId == undefined || ptetId == null
			|| ptetId == "" || ptetId == undefined) {
	...
	if (wrqId == null || wrqId == "" || wrqId == undefined)
		wrqId = "";
	...
	frmHtml
			.push("<form id='"
					+ targetFrm
					+ "' action='/dentViewer/popDentViewer.do' method='post' target='DentViewer'>");
	...
	function g_fGoMore(type, param, prgrStpDvsCod, dlvrScduD) {
		...
		if (type == 'mgrlst') {
			...
			inputVal = '<input type="hidden" name="kWrq_dvs_cod" value="' + param
					+ '" />';
			inputVal += '<input type="hidden" name="kPrgr_stp_dvs_cod" value="'
					+ prgrStpDvsCod + '" />';
			inputVal += '<input type="hidden" name="kDlvr_scdu_d" value="'
					+ dlvrScduD + '" />';
		} else if (type == 'makmgr') {
